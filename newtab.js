...
chrome.storage.local.get(['dailyStats', 'studyDomains', 'customNudges', 'allowances', 'tempFocusLog', 'tempFocusPasses'], (data) => {
  ...
  const { computeFocusBreakdown, focusScore } = self.HocusFocusMath;
  const { focusSeconds, distractionSeconds, sites, domains: domainTimes } =
    computeFocusBreakdown([today], {
      dailyStats: stats, studyDomains, allowances, tempFocusLog, tempFocusPasses
    });

  const score = focusScore(focusSeconds, distractionSeconds);
